...
class EnableTriggerTests: XCTestCase {
    ...
    func testEnableTrigger_success() {
        ...
        api.enableTrigger(target, triggerID: expectedTriggerID, enable: true) { (trigger, error) -> Void in
            if error == nil{
    ...
    func testEnableTrigger_404_error() {
        ...
        do{
            ...
            api.enableTrigger(target, triggerID: triggerID, enable: false, completionHandler: { (trigger, error) -> Void in
                if error == nil{
                    XCTFail("should fail")
                }else {
                    switch error! {
